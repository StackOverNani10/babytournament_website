...
export const SectionIndicator = ({ sections, theme }: SectionIndicatorProps) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      console.log('Scroll position:', window.scrollY, 'Window height:', windowHeight, 'Document height:', documentHeight);
      ...
      for (let i = 0; i < sections.length; i++) {
        ...
        if (element) {
          ...
          if (isInView) {
            console.log('Section in view:', section, {elementTop, elementBottom, scrollPosition});
        ...
        {sections.map((section, index) => (
          <button
            ...
            className={`relative flex items-center justify-center w-6 h-6 group transition-all duration-300 ${
              activeSection === section ? 'scale-125' : 'opacity-60 hover:opacity-100'
            }`}
